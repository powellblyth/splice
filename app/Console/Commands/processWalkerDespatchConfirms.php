                            ...
                            if (1 < count($data))
                            {
                                ...
                                else
                                {
                                    ...
                                    foreach ($headers as $index => $value)
                                    {
                                        ...
                                        $indexedWalkerData[$value] = $data[$index];
                                    }

                                    $orderNumber = $indexedWalkerData['OrderNumber'];
                                    ...
                                    try
                                    {
                                        ...
                                        try
                                        {
                                            $orderLine = $order->order_lines()->where(
                                                'line_number',
                                                $indexedWalkerData['DatabaseLineNumber']
                                            )->firstOrFail()
                                            ;
//                                        $orderLine = OrderLine::where('order_id', $order->id)->where('line_number', $indexedData['DatabaseLineNumber'])->first();
                                            ...
                                            $shipment = Shipment::firstOrCreate(
                                                [
                                                    'carrier' => $indexedWalkerData['Carrier'], 'tracking_number',
                                                    $indexedWalkerData['TrackingNumber']
                                                ],
                                            );

                                            if ( !$shipment->populateFromWalker($orderLine, $indexedWalkerData))
                                            {
                                                $errors[$orderNumber] = $fileName.' - could not save status of shipment order line '.$orderLine->line_number.' ('.$orderLine->product_code.') in database when marking despatched! Panic! '.print_r(
                                                        $indexedWalkerData,
                                                        true
                                                    );
                                            }
                                        ...
                                        {
                                            $errors[$orderNumber] = $fileName.' - order line '.$indexedWalkerData['DatabaseLineNumber'].' does not exist for order '.$order->id;
                                        }
                                    }
                                    catch (ModelNotFoundException $e)
                                    {
